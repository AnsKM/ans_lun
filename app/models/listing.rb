...
class Listing < ApplicationRecord
  ...
  reverse_geocoded_by :from_address_latitude, :from_address_longitude
  ...
  has_one :from_check_address, -> { where('listing_location_type = ?', 'from_check_address') }, :class_name => "Location", :dependent => :destroy
  ...
  accepts_nested_attributes_for :origin_loc, :destination_loc, :from_check_address
  ...
  def init_origin_location(location)
    if location.present?
      ...
      build_from_check_address(location.attributes)
    else
      build_origin_loc()
      build_from_check_address()
    end
  end
  ...
  def ensure_from_address
    build_from_check_address unless from_check_address
  end
