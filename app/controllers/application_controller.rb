...
class ApplicationController < ActionController::Base
  ...
  def ensure_logged_in(warning_message)
    @current_user = Person.find_by(id: params[:person_id]) || Person.find_by(username: params[:person_id]) if params[:person_id].present?
    if logged_in?
      true
    else
      if params[:listing].present?
        session[:listing] = params[:listing]
        session[:custom_fields] = params[:custom_fields]
        session[:listing_images] = params[:listing_images]
        session[:listing_ordered_images] = params[:listing_ordered_images]
      elsif params[:travel_detail].present?
        session[:travel_detail] = params[:travel_detail]
        session[:travel_single_date] = params[:travel_single_date]
        session[:travel_round_date] = params[:travel_round_date]
      end
