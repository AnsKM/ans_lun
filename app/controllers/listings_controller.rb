class ListingsController < ApplicationController
  ...
  #before_action :is_authorized_to_post, :only => [:create]
  ...
  def create
    if params[:custom_field_values].present?
      a = {}
      params[:custom_field_keys].each_with_index do |pr, index|
        a = a.merge(pr => params[:custom_field_values][index])
      end
      params.merge!(custom_fields: a)
    end

    if new_listing_author != @current_user
      logger.info "ADMIN ACTION: admin='#{@current_user.id}' create listing params=#{params.inspect}"
    end
    if show_location? && params[:listing][:origin_loc_attributes].present? && params[:listing][:origin_loc_attributes][:address].blank?
      params[:listing].delete("origin_loc_attributes")
    elsif show_location? && params[:listing][:from_check_address_attributes].present? && params[:listing][:from_check_address_attributes][:address].blank?
      params[:listing].delete("from_check_address_attributes")
    end

    shape = get_shape(Maybe(params)[:listing][:listing_shape_id].to_i.or_else(nil))
    listing_uuid = UUIDUtils.create
    cordinates = Geocoder.coordinates(params[:listing][:from_address])
    params[:listing][:from_address_latitude] = cordinates[0] if cordinates
    params[:listing][:from_address_longitude] = cordinates[1] if cordinates
    ...
    if params[:custom_field_keys].present?
      ListingImage.where(id: params[:listing_images].collect { |h| h[:id] }.select { |id| id.present? } ).update_all(author_id: @current_user.id)
    end
  ...
  def form_content
    make_listing_presenter
    if @listing.new_record? && (params[:action].eql?('new') || params[:action].eql?('new_form_content'))
      @listing.init_origin_location(nil)
    else
